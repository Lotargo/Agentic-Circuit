...
> **Инференс обязан работать как на GPU, так и на CPU.** Контейнеры TEI/ColBERT
> собираются с поддержкой обоих режимов: при наличии `CUDA/GPU` (через
> `deploy.resources.reservations.devices` или `runtime: nvidia`) грузят модель на GPU,
> иначе автоматически откатываются на CPU. Выбор режима — через env/флаг запуска, без
> пересборки образа. Локальный запуск без GPU должен работать на CPU из коробки.
...
## 13. Переносимость (учитывается заранее, НЕ цель)

**Финальная цель проекта — локальный запуск** всего стека через `docker-compose`.
Развёртывание в облаке — это потенциальная задача, которая может появиться когда-либо
позже; она не является целью текущей реализации. Контейнерная изоляция и HTTP/gRPC
границы выбраны так, чтобы такая миграция была *возможна*, но не обязательна.

Пример возможного будущего деплоя (не обязательство):
...
- **Облачные зависимости**: Qdrant заменяется на **Qdrant Cloud**; инференс-сервисы
  (TEI embedding / ColBERT rerank) переносятся на облачный инференс; MongoDB — на Atlas.
- OpenWebUI при деплое конфигурируется на URL TS-шлюза.

> Контракты между сервисами (URL/порты) берутся только из env, чтобы один и тот же образ
> работал и локально, и (теоретически) в облаке. Локальный запуск — приоритет №1.
