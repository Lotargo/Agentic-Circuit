...
- [ ] Общая Docker-сеть для всех сервисов
- [ ] Сервис Qdrant (отдельный контейнер; в проде -> Qdrant Cloud)
- [ ] Сервис MongoDB (отдельный контейнер)
- [ ] Сервис OpenWebUI (отдельный контейнер, -> TS-шлюз)
- [ ] Сервис TEI embedding (отдельный контейнер)
- [ ] Сервис ColBERT rerank (отдельный контейнер)
- [ ] Сервис Python-движок (отдельный контейнер, langserve; цель деплоя -> Render)
- [ ] Сервис TS-шлюз (отдельный контейнер; цель деплоя -> Vercel)
- [ ] HTTP/gRPC границы общения между сервисами
