...
def test_router_messages_ask_for_decision():
    msgs = router_messages("Привет, как дела?")
...
def test_phase2_sees_only_its_own_phase1():
    ...
    user = msgs[1]["content"]
    # phase2 must receive its own phase1 text, but NOT other circuits' content
...
def test_synthesis_messages_aggregate_all_circuits_and_treat_as_own():
    ...
    user = msgs[1]["content"]
    for c in ("creative", "pragmatic", "effective"):
        assert c in user
